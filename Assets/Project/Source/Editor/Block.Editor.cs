...
namespace Exa.CustomEditors
{
    ...
    public class BlockEditor : Editor
    {
        ...
        {
            private string name;
            private IEnumerable<FieldInfo> properties;

            public BlockDataInfo(Type dataType) {
                this.name = dataType.Name;
                this.properties = dataType.GetFields().ToList();
            }
            ...
            private object DrawField(string name, object c, Type cType) {
                if (cType == typeof(bool)) return EditorGUILayout.Toggle(name, (bool) c);
                if (cType == typeof(float)) return EditorGUILayout.FloatField(name, (float) c);
                return null;
            }
        }
    }
